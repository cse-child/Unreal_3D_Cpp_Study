...
void UCUserWidget_CrossHair::NativeOnInitialized()
{
	...
	for(UWidget* widget : widgets)
	{
		...
		// 변환 과정을 거쳐야 Canvas안에 있는 위젯의 속성값을 변경시킬 수 있다.
...
void UCUserWidget_CrossHair::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	for(int32 i = 0; i < (int32)EDirection::Max; i++)
	{
		...
		case EDirection::Top:
		case EDirection::Bottom:
			slot->SetAlignment(FVector2D(Alignments[i].X, value)); // Y값만 변경
			break;

		case EDirection::Left:
		case EDirection::Right:
			slot->SetAlignment(FVector2D(value, Alignments[i].Y)); // X값만 변경
			break;
		}
	}
}
