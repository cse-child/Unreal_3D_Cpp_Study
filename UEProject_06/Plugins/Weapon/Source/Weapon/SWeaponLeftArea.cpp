	...
	[
		...
		[
			...
			[
				...
				.OnTextChanged(this, &SWeaponLeftArea::OnTextChanged) // 글자가 바뀔때마다 호출
				.OnTextCommitted(this, &SWeaponLeftArea::OnTextCommitted) // 엔터 눌렀을 때 호출
			]
		]
		...
		[
			...
			.OnGenerateRow(this, &SWeaponLeftArea::OnGenerateRow) // 행을 등록할 때 호출
			.OnSelectionChanged(this, &SWeaponLeftArea::OnSelectionChanged) // 선택이 바뀔때마다 호출
		]
		+ SVerticalBox::Slot()
		.AutoHeight()
		[
			...
			[
				SNew(STextBlock)
				.Text(this, &SWeaponLeftArea::OnGetAssetCount) // Slate에서는 Text에 이벤트를 넣어줄 수 있다.
			]
		]
	];
...
void SWeaponLeftArea::ReadDataAssetList()
{
	...
	ListView->RequestListRefresh(); // List 갱신
}
